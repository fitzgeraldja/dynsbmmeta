...
def max_global_overlap_over_perms(Z_1,Z_2,Q):
    ...
    if -1 in vals:
        ...
        best_perm = max(perms(range(Q)),key=lambda perm: (np.array([-1]+list(perm))[idxs].reshape(Z_1.shape)==mZ_2).sum())
        overlaps = (np.array([-1]+list(best_perm))[idxs].reshape(Z_1.shape)==mZ_2).sum(axis=1).astype(np.double)/(Z_1!=-1).sum(axis=0)
    else:
        best_perm = max(perms(range(Q)),key=lambda perm: (np.array(perm)[idxs].reshape(Z_1.shape)==Z_2).sum())
        overlaps = (np.array(best_perm)[idxs].reshape(Z_1.shape)==Z_2).sum(axis=1).astype(np.double)/(Z_1.shape[1])
...
def max_local_overlap_over_perms(Z_1,Z_2,Q):
    ...
    for t in range(len(Z_1)):
        ...
        if -1 not in vals:
            best_perm= max(perms(range(Q)),key=lambda perm: (np.array(perm)[idxs].reshape(Z_1.shape[1])==Z_2[t,:]).sum())
            overlaps[t] = (np.array(best_perm)[idxs].reshape(Z_1.shape[1])==Z_2[t,:]).sum().astype(np.double)/Z_1.shape[1]
            best_perms[t,:]=best_perm
        else:
            mZ_2 = ma.masked_equal(Z_2,-1)
            best_perm= max(perms(range(Q)),key=lambda perm: (np.array([-1]+list(perm))[idxs].reshape(Z_1.shape[1])==mZ_2[t,:]).sum())
            overlaps[t] = (np.array([-1]+list(best_perm))[idxs].reshape(Z_1.shape[1])==mZ_2[t,:]).sum().astype(np.double)/(Z_1[t,:]!=-1).sum()
            best_perms[t,:]=best_perm
...
def overlap_given_perm(Z_1,Z_2,perm):
    ...
    for t in range(len(Z_1)):
        ...
        overlaps[t] = (np.array(perm)[idxs].reshape(Z_1.shape[1])==Z_2[t,:]).sum().astype(np.double)/Z_1.shape[1]
    return overlaps
...
def best_global_perm_from_mse(M_1,M_2):
    ...
    """
    Q = M_1.shape[1]
    best_perm = min(perms(range(Q)),key=lambda perm: np.mean(np.square(temporal_matrix_perm(M_1,perm)-M_2)))
...
def calc_flexibility(Z,i=None,method='base'):
    ...
    T = Z.shape[0]
    if method=='base':
        base_flex = np.hstack([Z[t-1,:]!=Z[t,:] for t in range(1,T)]).sum(axis=1)/T
        return base_flex
...
def jacc_idx(Z):
    ...
    T = Z.shape[0]
    Q = np.amax(Z)+1
    jaccs = np.array([[[len(set(np.nonzero(Z[t]==q)[0])&set(np.nonzero(Z[t+1]==r)[0]))
                        /len(set(np.nonzero(Z[t]==q)[0])|set(np.nonzero(Z[t+1]==r)[0])) 
                        for r in range(Q)] for q in range(Q)] for t in range(T-1)])
    return jaccs
...
def jacc_flex(Z):
    ...
    jaccs = jacc_idx(Z)
    T = Z.shape[0]
    jacc_flex = np.array([np.nanmean([jaccs[t,Z[t,i],Z[t+1,i]] if (Z[t,i]!=-1 and Z[t+1,i]!=-1) else np.nan for t in range(T-1)]) for i in range(Z.shape[1])])
