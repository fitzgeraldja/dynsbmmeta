...
def calcLL(partitions, edges, LL_fun=None):
    ...
    for pi, partition in enumerate(partitions):
        ...
        A1 = sparse.coo_matrix(
            (np.ones(m), (mi, ei)), shape=(K, m), dtype=np.uint
        ).tocsc()
        A2 = sparse.coo_matrix(
            (np.ones(m), (ei, mj)), shape=(m, K), dtype=np.uint
        ).tocsc()
        ...
        LL[pi, 0] = (
            e_rs * np.log(prs + 1e-200) + (nrns - e_rs) * np.log(1 - prs + 1e-200)
        ).sum() / 2.0
        ...
        prs[drds == 0] = 1e-10
        LL[pi, 1] = (e_rs * np.log(prs + 1e-200)).sum() / 2.0
        # ~ print pi,m,e_rs,e_rs.sum(), LL[pi]
        q = n - np.sum(partitions[0, :] == partition)
        LL[pi, 2:] = LL[pi, 1] + np.float64(
            [
                (q) * theta + (n - q) * (oneminustheta)
                for theta, oneminustheta in zip(thetas, oneminusthetas)
            ]
        )
...
def calcVI(partitions):
    ...
    for i in tqdm(range(num_partitions)):
        ...
        #     print(i)
        A1 = sparse.coo_matrix(
            (np.ones(n), (partitions[i, :], nodes)), shape=(c, n), dtype=np.uint
        ).tocsc()
        ...
        for j in range(i):
            ...
            A2 = sparse.coo_matrix(
                (np.ones(n), (nodes, partitions[j, :])), shape=(n, c), dtype=np.uint
            ).tocsc()
            ...
            nmat = np.divide(
                n12all * n12all,
                (np.outer(n1all, n2all)),
                where=np.outer(n1all, n2all) > 0,
            )
...
def run_partitions(
    partitions, edges, nreps=15, divisions=20, outFile=None, DC=False, LL_fun=None
):
    """
    ...
    if -1 in np.unique(partitions):
        raise ValueError(
            "-1 found in partition: remove non-present nodes before passing to this function"
        )
    c = len(np.unique(partitions))
    n = np.shape(partitions)[1]
    ...
    print(
        f"Aiming for {np.shape(partitions)[0]} partitions for {np.shape(partitions)[1]} nodes"
    )
    print(
        f"Including {n_random} fully random partitions with same number of groups as baseline"
    )
    ...
    print(
        f"Provided {n_partitions} original partitions, each of which will have sections \
