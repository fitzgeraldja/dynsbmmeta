...
plt.style.use(
    "/home/fitzgeraldj/Documents/main_project/confirmation/code/dynsbm_meta/python/proj1_package/src/proj1_package/proj1_7clrs.mplstyle"
)
...
def mask_network(A, mask_percent=5):
    ...
    sel_non_edges = np.random.choice(
        non_edges, size=len(non_edges) // (100 / mask_percent)
    )
    for t in range(T):
...
def zero_network(A, mask_percent_zs=5, mask_percent_nzs=5):
    ...
    z_idxs = [
        rng.choice(
            np.array(np.nonzero(A_zs[t])).T,
            replace=False,
            size=int(sz_zs[t] // (100 / mask_percent_zs)),
            axis=0,
        ).T
    ...
    nz_idxs = [
        rng.choice(
            np.array(np.nonzero(A_nzs[t])).T,
            replace=False,
            size=int(sz_nzs[t] // (100 / mask_percent_nzs)),
            axis=0,
        ).T
        for t in range(A.shape[0])
    ]
...
def perturb_metadata(X, metatypes=[], ps=[]):
    ...
    for s, p in enumerate(ps):
        ...
        if "categorical" in metatypes[s] or "bernoulli" in metatypes[s]:
            ...
            nXs[rX < p] = X[s][rX < p] + np.random.randint(
                -3, high=3, size=X[s][rX < p]
            )
            nXs[nXs < 0] = 0
        nX.append(nXs)
        mask_locs.append(mask_loc)
...
def predict_links(
    dynsbmmeta, top_k=None, edges=None, nodes=None, metadata=None, taum=None
):
    """
        ...
        if top_k is None:
            if taum is None:
                ...
                if len(dynsbmmeta.beta_mat.shape) == 2:
                    probs = np.array(
                        [
                            dynsbmmeta.beta_mat[dynsbmmeta.Z[t, i], dynsbmmeta.Z[t, j]]
                            for t, i, j in edges
                        ]
                    )
                    ...
                    probs = np.array(
                        [
                            dynsbmmeta.beta_mat[
                                t, dynsbmmeta.Z[t, i], dynsbmmeta.Z[t, j]
                            ]
                            for t, i, j in edges
                        ]
                    )
                            ...
                            np.sum(
                                [
                                    [
                                        taum[t, i, q]
                                        * taum[t, j, r]
                                        * dynsbmmeta.beta_mat[q, r]
                                        for r in range(dynsbmmeta.Q)
                                    ]
                                    for q in range(dynsbmmeta.Q)
                                ]
                            )
                            for t, i, j in edges
                        ]
            ...
            print(
                "Using top k evaluation for edges: note only Q^2 possible ranks (for directed case)\nor Q*(Q+1)/2 for undirected models"
            )
            true_in_topk = np.zeros((len(edges),))
            for m, (t, i, j) in enumerate(edges):
                probs = np.array(
                    [
                        dynsbmmeta.beta_mat[dynsbmmeta.Z[t, i], dynsbmmeta.Z[t, k]]
                        for k in range(dynsbmmeta.N)
                    ]
                )
    ...
    if taum is None:
        ...
        for node in nodes:
            ...
            for s, metatype in enumerate(metatypes):
                if metatype == "poisson":
                    expected_X[(t, nidx)].append(
                        np.floor(metaparams[s][t, dynsbmmeta.Z[t, nidx], 0])
                    )
                elif "bernoulli" in metatype or "categorical" in metatype:
                    expected_X[(t, nidx)].append(
                        np.argmax(metaparams[s][t, dynsbmmeta.Z[t, nidx], :])
                    )
        return expected_X
    else:
        metatypes = dynsbmmeta.metatypes
                ...
                if metatype == "poisson":
                    expected_X[(t, nidx)].append(
                        np.floor(
                            np.sum(
                                [
                                    metaparams[s][t, q, 0] * taum[t, nidx, q]
                                    for q in range(dynsbmmeta.Q)
                                ]
                            )
                        )
                    ...
                    expected_X[(t, nidx)].append(
                        np.argmax(
                            np.sum(
                                [
                                    taum[t, nidx, q] * metaparams[s][t, q, :]
                                    for q in range(dynsbmmeta.Q)
                                ],
                                axis=0,
                            )
