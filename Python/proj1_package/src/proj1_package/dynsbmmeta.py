...
class Dynsbmmeta_base(object):
    def __init__(
        ...
        self.params = None
        self.metatypes = (
            metatypes  # assume list length S with string names of metadata type
        )
        ...
        if (metatype == "poisson") | (metatype == "negative binomial"):
            if q is None:
                ...
                for q_ in range(self.Q):
                    ...
                    x = np.linspace(
                        metadists[q_].ppf(0.001), metadists[q_].ppf(0.999), num=100
                    )
                    ax.plot(
                        x,
                        metadists[q_].pdf(x),
            ...
            if q is None:
                ...
                for q_ in range(self.Q):
                    ...
                    rects1 = ax.bar(
                        x - width / 2,
                        [metadists[q_][x_] for x_ in x],
                        width,
                        label="Estimate",
                    )
                    rects2 = ax.bar(
                        x + width / 2,
                        float(np.sum(metadata, axis=1)) / len(metadata[:, 0]),
                        width,
                        label="Observed",
                    )
                ...
                width = 0.35
                rects1 = ax.bar(
                    x - width / 2, [metadist[x_] for x_ in x], width, label="Estimate"
                )
                rects2 = ax.bar(
                    x + width / 2,
                    float(np.sum(metadata, axis=1)) / len(metadata[:, 0]),
            ...
            if q is None:
                ...
                for q_ in range(self.Q):
                    ...
                    f = np.reshape(
                        metadists[q_].pdf(pca.inverse_transform(posns)), xx.shape
                    )
    ...
    def chisq_test(self, metatype=None, t=0, q=0):
        ...
        chisq, p_val = chisquare(
            val_counts.values(),
            f_exp=len(metadata)
            * np.array([self.metadist[s][t][q].pdf(x) for x in val_counts.keys()]),
            ddof=len(self.metaparams[s][t][q]),
        )
        return chisq, p_val
        ...
        for k in range(K):
            for l in range(K):
                if k == 0:
                    if l == 0:
                        A[: sizes[0], : sizes[0]] = (
                            A[: sizes[0], : sizes[0]] <= p_in
                        ) * 1.0
                    else:
                        A[: sizes[0], cum_size[l - 1] : cum_size[l]] = (
                            A[: sizes[0], cum_size[l - 1] : cum_size[l]] <= p_out
                        ) * 1.0
                else:
                    if l == 0:
                        A[cum_size[k - 1] : cum_size[k], : sizes[0]] = (
                            A[cum_size[k - 1] : cum_size[k], : sizes[0]] <= p_out
                        ) * 1.0
                    else:
                        if k == l:
                            A[
                                cum_size[k - 1] : cum_size[k],
                                cum_size[l - 1] : cum_size[l],
                            ] = (
                                A[
                                    cum_size[k - 1] : cum_size[k],
                                    cum_size[l - 1] : cum_size[l],
    ...
    def evolve_Z(self, Z_1, trans_prob, T):
        ...
        for i in range(T - 1):
            Z[:, i + 1] = np.array(
                [self.pick_category(trans_prob[int(zi), :], 1) for zi in Z[:, i]]
            )
        return Z
    ...
    def sample_dynsbm_meta(
        ...
        for i, meta_type in enumerate(meta_types):
            ...
            if meta_type == "normal":
                ...
                X = [
                    [
                        poisson.rvs(params[0, q, t], size=(sizes[q, t],))
                        for q in range(Q)
                    ]
                    for t in range(T)
                ]
                ...
                X = [
                    [
                        nbinom.rvs(
                            params[0, q, t], params[1, q, t], size=(sizes[q, t],)
                        )
                        for q in range(Q)
                    ]
                    for t in range(T)
                ...
                X = [
                    [
                        np.array(
                            [
                                bernoulli.rvs(params[l, q, t], size=(sizes[q, t],))
                                for l in range(L)
                            ]
                        )
                        for q in range(Q)
                ...
                X = [
                    [self.pick_category(params[:, q, t], sizes[q, t]) for q in range(Q)]
                    for t in range(T)
                ]
    ...
    def rmi(self, true=None, data=None, verbose=False, method="within", show_nmi=False):
        ...
        if method == "total":
            ...
            if np.amax(data[:, 0]) != R - 1:
                ...
                if verbose:
                    print("Inferred: ", *set(data[:, 0]))
                _, data[:, 0] = np.unique(data[:, 0], return_inverse=True)
                if verbose:
                    print("Relabelled: ", *set(data[:, 0]))
            ...
            for k in range(n):
                r, s = data[k]
                r = (
                    R - 1 if type(r) is type(None) else r
                )  # nb this treats non-present nodes as other community
            ...
            logOmega = (
                (R - 1) * (S - 1) * np.log(n + 0.5 * R * S)
                + 0.5 * (R + nu - 2) * np.sum(np.log(y))
                + 0.5 * (S + mu - 2) * np.sum(np.log(x))
                + 0.5
                * (
                    gammaln(mu * R)
                    + gammaln(nu * S)
                    - R * (gammaln(S) + gammaln(mu))
                    - S * (gammaln(R) + gammaln(nu))
                )
            )
            ...
            for t in range(self.T):
                ...
                if -1 in np.unique(datat):
                    datat = datat[datat != -1].reshape(
                        ((datat != -1).sum(axis=0)[0], 2)
                    )
                n = len(datat)
                R = len(set(datat[:, 0]))
                S = len(set(datat[:, 1]))
                ...
                if verbose:
                    ...
                    print()
                    print(
                        "Estimated number of contingency tables Omega =", exp(logOmega)
                    )
                    print()
                    # Print the reduced mutual information
                    print(
    ...
    def nrmi(self, data=None, true=None, method="within", **kwargs):
        ...
        if method == "total":
            ...
            pi1s = self.rmi(
                data=np.tile(data[:, :, 0], (2, 1, 1)).transpose((1, 2, 0)),
                method=method,
            )
            pi2s = self.rmi(
                data=np.tile(data[:, :, 1], (2, 1, 1)).transpose((1, 2, 0)),
                method=method,
            )
            ...
            return np.array(
                [
                    2 * redmi / (pi1 + pi2)
                    for redmi, pi1, pi2 in np.column_stack((redmis, pi1s, pi2s))
                ]
            )
    ...
    def sankey(self, flow_type="flow", title=None):
        ...
        comms = {
            t: [
                [i for i, x in enumerate(self.Z[t, :]) if x == q]
                + [i for i, x in enumerate(self.Z[t, :]) if x == -1]
        ...
        for t in range(self.T - 1):
            for cm in comms[t]:
                raw_flows.append(
                    [len(set(cm).intersection(set(com))) for com in comms[t + 1]]
                )
                flows.append(
                    [
                        len(set(cm).intersection(set(com))) / len(cm)
                ...
                jaccard.append(
                    [
                        len(set(cm).intersection(set(com)))
                        / len(set(cm).union(set(com)))
                        if len(set(cm).union(set(com))) != 0
                        else 0.0
                        for com in comms[t + 1]
                    ]
                )
        ...
        data = pd.DataFrame(
            proper_list,
            columns=["source", "target", "raw_flow", "flow", "label", "jaccard"],
        )
        ...
        colours = [
            to_hex(cmap(i % max(data["target"]))) if i not in outside_net else "#ffffff"
            for i in all_comms
        ]
